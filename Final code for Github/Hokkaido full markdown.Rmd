...
rownames(Invert)<-as.character(Invert[,29]) # set rownames as unique mesocosm IDs
Invert<-Invert[,c(-1:-5,-25:-29)] # remove all but the species abundance columns
...
ggplot(data = C.Comp,
  ...
  ylab("Macroinvertebrate taxon richness") + 
  xlab("Predator treatment") + 
  theme_classic() +
  theme(text = element_text(size = 16), 
        axis.text = element_text(colour = "black"),
...
Now we'll ask how predator and heatwave treatment affected macroinvertebrate density. Note that our mesocosms are 3.5 L in volume, so we will divide total abundance by 35 to get the density of our macroinvertebrates per cubic meter:

```{r}

##-- End of recovery period --##
# hist(C.Comp$`Total Abundance`/35) # assess normality (not run)
summary(aov((C.Comp$`Total Abundance`/35)~C.Comp$Pred.Treatment*C.Comp$Heat.Treatment)) # run anova
# plot(aov((C.Comp$`Total Abundance`/35)~C.Comp$Pred.Treatment*C.Comp$Heat.Treatment)) # check assumption of homoscedacity (not run)
TukeyHSD(aov((C.Comp$`Total Abundance`/35)~C.Comp$Pred.Treatment*C.Comp$Heat.Treatment)) # Tukey's HSD post-hoc test checks significance of group differences
...
Invert_plot3<-ggplot(data = C.Comp,
                     mapping = aes(x = C.Comp$Pred.Treatment,
                                   y = C.Comp$`Total Abundance`/35),
                     group=C.Comp$Heat.Treatment) +
                ylab("Macroinvertebrate Density 100 (cm^3)") + 
                xlab("Predator treatment") + 
                theme_classic() +
                theme(text = element_text(size = 16), 
                      axis.text = element_text(colour = "black"),
...
ggplot(data = Leaf,
  ...
  facet_grid(. ~ Leaf$Heat.Treatment,
             scales = 'fixed') + 
  ylim(min(Leaf$micro.degred.factor)-0.005,max(Leaf$micro.degred.factor)+0.005) + 
  scale_shape_manual(values = c(16,17),
                     guide=FALSE) +
  scale_fill_viridis_d(end = 0.8,begin = 0.3,
                         option = "inferno")
...
So finally we'll test for treatment effects on leaf shredding by macroinvertebrates, using our corrected values (accounting for microbial degradation, where negative values indicate an interaction between macroinvertebrate and microbial breakdown; see Methods):
...
Invert_plot4<-ggplot(data = Leaf,
                ...
                facet_grid(. ~ Leaf$Heat.Treatment,
                           scales = 'fixed') + 
                ylim(min(Leaf$Corrected.shred)-0.02,max(Leaf$Corrected.shred)+0.02) + 
                scale_shape_manual(values = c(16,17),
                                   guide=FALSE) +
                scale_fill_viridis_d(end = 0.8,begin = 0.3,
                                       option = "inferno")
